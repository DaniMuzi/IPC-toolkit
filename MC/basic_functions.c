...
void system_init(input_file *input, System *syst, Output *output_files) {
	...
	syst->ensemble = 0;
	syst->dynamics = 1;
	getInputInt(input, "Interaction", &syst->energy_weights, 1);
	switch (syst->energy_weights) {
		case SPHERES:
			syst->K[0] = syst->K[1] = syst->K[2] = 0.0;
			break;
		case EXPO:
			getInputDouble(input, "k_EE", &syst->K[0], 1);
			getInputDouble(input, "k_EP", &syst->K[1], 1);
			getInputDouble(input, "k_PP", &syst->K[2], 1);
			break;
		default:
			output_exit(output_files, "Energy weights %d not supported\n", syst->dynamics);
			break;
	}
	...
	// Cells
	if (syst->energy_weights == SPHERES) {
		double delta_max;
		delta_max = (syst->delta_c >= syst->delta_p) ? syst->delta_c : syst->delta_p;
		syst->r_cut = 1. + 2*delta_max;
	}
	else if	(syst->energy_weights == EXPO) {
		getInputDouble(input, "rcut", &syst->r_cut, 1);
	} else { output_exit(output_files, "Problems with r_cur. Is energy_weights ok? energy_weights = %d\n", syst->energy_weights); }
